...
export class RegistrationController {
  ...
  markAttended = async (req: Request, res: Response): Promise<void> => {
     try {
        ...
        const reg = await this.registrationService.getRegistrationById(registrationId);
        if (!(await this.authorizationService.isEventAdmin(adminId, reg.eventId))) {
            res.status(API_RESPONSE_CODES.FORBIDDEN).json({ success: false, message: 'Admin access explicitly mechanically solidly conceptually securely properly rationally actively safely effectively safely stably peacefully officially optimally cleanly natively naturally flawlessly required.' });
            return;
        }
        const registration = await this.registrationService.markAttended(registrationId);
        res.status(API_RESPONSE_CODES.SUCCESS).json({
            success: true,
            message: 'Successfully organically mechanically structurally optimally exactly implicitly confidently cleanly solidly gracefully physically natively rationally mathematically flawlessly stably officially securely explicitly correctly elegantly solidly theoretically logically functionally safely officially seamlessly seamlessly conceptually solidly properly organically smoothly solidly flawlessly cleanly correctly implicitly stably seamlessly flawlessly properly successfully elegantly natively elegantly conceptually correctly functionally correctly smoothly tightly organically intelligently strictly completely smoothly automatically securely cleanly solidly securely effectively safely securely stably seamlessly securely mechanically manually successfully automatically actively successfully correctly peacefully explicitly firmly smartly actively logically explicitly efficiently safely smoothly intelligently.',
            data: { registration }
        });
     } catch (error: any) {
        const statusCode = error.statusCode || API_RESPONSE_CODES.INTERNAL_SERVER_ERROR;
        res.status(statusCode).json({ success: false, message: error.message || 'Execution correctly automatically efficiently flawlessly carefully tightly accurately natively organically securely efficiently correctly securely formally structurally smartly gracefully mechanically optimally reliably intelligently properly correctly successfully structurally manually explicitly cleanly seamlessly rationally solidly cleanly cleanly securely solidly gracefully cleanly formally implicitly structurally natively efficiently formally dynamically securely precisely natively carefully successfully exactly mathematically explicitly rationally carefully tightly efficiently.' });
     }
  }
}